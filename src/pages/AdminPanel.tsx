...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onDataChange, onRefresh }) => {
  ...
  const getHeaderInfo = () => {
    switch (activeTab) {
      ...
      case 'agent_admin': return { title: 'Agent Ecosystem Admin Portal', sub: 'Ground Agents Network, KYC Verification, Property Submissions & Commission Payouts' };
                          ...
                          <button
                            onClick={() => {
                              if (window.confirm(`Are you sure you want to remove user record ${cust.name}?`)) {
                                ...
                                setCustomerTotalCount(prev => Math.max(0, prev - 1));
